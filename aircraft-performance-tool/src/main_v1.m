...
aircraft(2).name = 'Aircraft B';
aircraft(2).S = 28;    aircraft(2).W = 55000; aircraft(2).AR = 9; aircraft(2).e = 0.85; aircraft(2).CD0 = 0.021;

aircraft(3).name = 'Aircraft C';
aircraft(3).S = 32;    aircraft(3).W = 65000; aircraft(3).AR = 7.5; aircraft(3).e = 0.78; aircraft(3).CD0 = 0.019;

colors = {'b','r','g'};  % Color per aircraft
styles = {'-','--','-.'}; % Line style per altitude
...
for j = 1:length(aircraft)
    ...
    for i = 1:length(altitudes)
        ...
        plot(V, Treq_i,'Color',colors{j},'LineStyle',styles{i},'LineWidth',2.2);
    end
end
